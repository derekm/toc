...
|_Scale_
|Adding cluster nodes rebalances stream segment containers across nodes. All components are deployed in a horizontally scalable manner. Metadata that requires write scalability is stored within Pravega itself.
...
|_Write latency_
|1. Writer appends to segment store.

2. Segment store: throttling (if needed) + durable data log writes. Write & ack quorum sizes control durable data log striping and replication.

3. Segment store acks back to writer.
...
|_Transactions_
|Resumable transactions span stream segments. Transaction segments append to stream segments upon commit, preventing the need for reader isolation levels. Transaction segments are created for all stream segments, even if unused.
